...
2. **RESOLVED (v3 physics revision, 2026-07-17): scalar-grating validity limit.** The 2026-07-13 RCWA audit found that at the TIR-mandated PMMA periods (430–449 nm < λ), scalar diffraction theory overestimates the first-order coupling efficiency badly: at the v2 record design (period 438 nm, depth 400 nm), scalar η₁ = 0.347 at 532 nm vs rigorous RCWA 0.068 unpolarized (~5×), and ~15× at 635 nm — see `results/design_rcwa_check_na.csv` and `metagrating/rcwa_depth_sweep.csv`. RCWA also moves the depth optimum to ~200 nm (rigorous η peaks there; scalar keeps climbing to the 400 nm bound). **The v3 engine now interpolates η₁(period, depth, duty; λ, polarization) from a rigorous `grcwa` grid** (`physics/rcwa_eta_grid.npz`, built by `physics/calibrate_rcwa.py`, off-grid interpolation error audited in `results/rcwa_calibration_check.csv`), so absolute transmission, polarization responses, and the depth optimum are quoted at the rigorous (L2) level *inside the design loop itself*. Records/tables are keyed on the engine version (`best_design_ever_v3.csv`, `surrogate_runs_v3.csv`, …); v2-era files are archived, not overwritten. The checkpoint physics-probe system automatically refuses v2-trained surrogates and forces retraining — that is by design.
